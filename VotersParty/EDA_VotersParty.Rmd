...
dfRawTrain[vFeaturesDemographic] %>% subset(YOB > 2003)
dfRawTrain[vFeaturesDemographic] %>% subset(YOB < 1930)
...
# Two-way tables, Stacked barplots & Mosaic Plots
######################################################################################################
vFeaturesCat <- c("Gender", "EducationLevel", "Income", "HouseholdStatus")
lapply(dfRawTrain[vFeaturesCat], FUN = function(x) table(x, dfRawTrain[["Party"]], useNA = "ifany"))

plotStackedBar <- function(dfInput, feature_cat1, feature_cat2) {
    df <- data.frame(x = dfInput[[feature_cat1]], y = dfInput[[feature_cat2]])
    p <- ggplot(df) +
        geom_bar(aes(x = x, y = ..count.., fill = y)) +
        scale_fill_manual(values = c("blue","red"), name = feature_cat2) +
        xlab(feature_cat1) +
        ggtitle(paste("Stacked barplot of", feature_cat1, "w.r.t.", feature_cat2)) +
        theme(axis.text.x = element_text(angle = 45, vjust = 0.5))
    
    return(p)
}

plotMosaic <- function(dfInput, feature_cat1, feature_cat2) {
    df <- data.frame(x = dfInput[[feature_cat1]], y = dfInput[[feature_cat2]])
    vColors <- c("blue","red")
    p <- with(df, mosaicplot(x ~ y, shade=F, color = vColors,
                             xlab=feature_cat1, ylab=feature_cat2, 
                             main=paste("Voting by", feature_cat2)))
    
    return(p)
}

for(feature in vFeaturesCat) {
    print(plotStackedBar(dfRawTrain, feature, "Party"))
    print(plotMosaic(dfRawTrain, feature, "Party"))
}
