...
model_kNN <- function(dfTrain_fold, dfTest_fold, seedForFolds) {
    ...
    pred_oof <- predict(fit_kNN, newdata = dfTest_fold %>% select(-starts_with(ID_VAR)))
    pred_test <- predict(fit_kNN, newdata = dfTest %>% select(-starts_with(ID_VAR)))
...
dfPred_train <- dfPred_train %>% 
    set_colnames(paste("kNN", preProcessFuncName, vSeeds, sep = "_")) %>% 
    rownames_to_column(var = ID_VAR)
write.csv(dfPred_train, file = paste0("ensemble_kNN_train_", preProcessFuncName, ".csv"), row.names = F, quote = F)

dfPred_test <- do.call(cbind.data.frame, lPred_test)
dfPred_test <- dfPred_test %>% 
    set_colnames(paste("kNN", preProcessFuncName, vSeeds, sep = "_")) %>% 
    rownames_to_column(var = ID_VAR)
write.csv(dfPred_test, file = paste0("ensemble_kNN_test_", preProcessFuncName, ".csv"), row.names = F, quote = F)
...
write.csv(dfPred_test[c(ID_VAR, "PREDICTIONS")], 
          file = paste0("submission_kNN_avg_", preProcessFuncName, ".csv"), row.names = F, quote = F)
